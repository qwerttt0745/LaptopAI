...
This repository is designed as a full DevOps portfolio project: application code, CI/CD, Kubernetes manifests, and Terraform infrastructure in one place.
...
After infrastructure is ready, deployments are performed directly by GitHub Actions using Helm charts and `helm upgrade --install`.
...
flowchart LR
       ...
       C --> D[Build Images]
       D --> E[Push to AWS ECR]
       E --> F[GitHub Actions Deploy via Helm]
       F --> H[k3s Cluster on EC2]
       H --> I[Frontend]
       H --> J[Backend API]
       H --> K[AI Service]
       J --> L[(PostgreSQL RDS)]
...
├── k8s/                    # Helm charts and Kubernetes configs
...
4. GitHub Actions runs `helm upgrade --install` for backend, frontend, and ai-service.
5. Rollout status is checked for each deployment.
...
flowchart TD
       ...
       A --> C[lint-frontend]
       B --> D[test-backend]
       C --> E[build-push-and-deploy]
       D --> E
       E --> F[Helm rollout checks]
```
...
3. Monitoring is optional to avoid memory pressure on small instances.
...
2. Direct CI/CD deployment model with GitHub Actions and Helm.
